...
test_loss_arr = np.zeros((17*11, 4))

for channels in np.linspace(1, 64, 17, dtype=np.int32):
    for linear_nodes in np.linspace(1, 1000, 11, dtype=np.int32): # change range based on whether adjusting channels or layers
        net = conv_net(channels, linear_nodes).to(device)
        net_cpu = conv_net(channels, linear_nodes).to(device_cpu) 
        ...
        loss_path = "./performance/loss_base_{}_{}.npy".format(channels, linear_nodes)
        prediction_actual_path = "./performance/prediction_actual_base_{}_{}.npy".format(channels, linear_nodes)
        ...
        test_loss_arr[i, 1] = channels
        test_loss_arr[i, 2] = linear_nodes
        test_loss_arr[i, 3] = total_params

        np.save("./performance/test_loss_parameters", test_loss_arr)
